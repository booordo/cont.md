    ...
    _createClass(Puzzle, [{
      ...
      value: function _updateSize() {
        ...
        if (clientWidth < this.width) {
          this.mainGroup.transform("T ".concat(offsetX, ", ").concat(offsetY, " s").concat(k));
          this.context.attr({
            style: "height: ".concat(this.contextHeight * k, "px")
          });
        } else {
          this.mainGroup.transform("T ".concat(offsetX, ", ").concat(offsetY));
          this.context.attr({
            style: "height: 800px"
          });
        }
      ...
      value: function _createCircle() {
        ...
        bottomText.transform("T ".concat(this.cx - bottomTextBBox.cx, " 0")); // IE FIX <--
      ...
      value: function _createSector(fromAngle, toAngle, element) {
        ...
        var sector = this.context.path("M ".concat(oArcPts.start.x, " ").concat(oArcPts.start.y, "\n            A ").concat(this.outerRadius, " ").concat(this.outerRadius, " 0 0 1 ").concat(oArcPts.end.x, " ").concat(oArcPts.end.y, "\n            L ").concat(oConPts.start.x, " ").concat(oConPts.start.y, "\n            C ").concat(oConPts.guides.start.x, " ").concat(oConPts.guides.start.y, " ").concat(oConPts.guides.end.x, " ").concat(oConPts.guides.end.y, " ").concat(oConPts.end.x, " ").concat(oConPts.end.y, "\n            L ").concat(iArcPts.end.x, " ").concat(iArcPts.end.y, "\n            A ").concat(this.innerRadius, " ").concat(this.innerRadius, " 0 0 0 ").concat(cConPts.end.x, " ").concat(cConPts.end.y, "\n            C ").concat(cConPts.guides.end.x, " ").concat(cConPts.guides.end.y, " ").concat(cConPts.guides.start.x, " ").concat(cConPts.guides.start.y, " ").concat(cConPts.start.x, " ").concat(cConPts.start.y, "\n            A ").concat(this.innerRadius, " ").concat(this.innerRadius, " 0 0 0 ").concat(iArcPts.start.x, " ").concat(iArcPts.start.y, "\n            L ").concat(iConPts.end.x, " ").concat(iConPts.end.y, "\n            C ").concat(iConPts.guides.end.x, " ").concat(iConPts.guides.end.y, " ").concat(iConPts.guides.start.x, " ").concat(iConPts.guides.start.y, " ").concat(iConPts.start.x, " ").concat(iConPts.start.y, "\n            Z")).attr({
          fill: "rgba(255,255,255,0.4)",
          stroke: "#fff",
          strokeWidth: "3px",
          class: "puzzle__sector"
        });
        this.circlePath.unshift("A ".concat(this.innerRadius, " ").concat(this.innerRadius, " 0 0 0 ").concat(cConPts.end.x, " ").concat(cConPts.end.y, "\n            C ").concat(cConPts.guides.end.x, " ").concat(cConPts.guides.end.y, " ").concat(cConPts.guides.start.x, " ").concat(cConPts.guides.start.y, " ").concat(cConPts.start.x, " ").concat(cConPts.start.y, "\n            A ").concat(this.innerRadius, " ").concat(this.innerRadius, " 0 0 0 ").concat(iArcPts.start.x, " ").concat(iArcPts.start.y));

        if (this.elements.indexOf(element) == this.elements.length - 1) {
          this.circlePath.unshift("M ".concat(iArcPts.end.x, " ").concat(iArcPts.end.y));
        }

        group.add(sector);
        element.title.forEach(function (el, i) {
          var textStartPoint = _this2._getPoint(_this2.cx, _this2.cy, _this2.middleRadius - (middleAngle > 180 ? i * 20 : -i * 20), fromAngle);

          var textEndPoint = _this2._getPoint(_this2.cx, _this2.cy, _this2.middleRadius - (middleAngle > 180 ? i * 20 : -i * 20), toAngle);

          var text = _this2.context.text(0, 0, el).attr({
            class: "puzzle__text",
            "textpath": "M ".concat(middleAngle > 180 ? textStartPoint.x + " " + textStartPoint.y : textEndPoint.x + " " + textEndPoint.y, " \n                A ").concat(_this2.middleRadius, " ").concat(_this2.middleRadius, " 0 0 ").concat(middleAngle < 180 ? 0 : 1, " ").concat(middleAngle > 180 ? textEndPoint.x + " " + textEndPoint.y : textStartPoint.x + " " + textStartPoint.y)
          });
      ...
      value: function _sectorHoverInHandler(e) {
        ...
        Snap.animate(1, 1.2, function (val) {
          _this3.attr({
      ...
      value: function _sectorHoverOutHandler(e) {
        ...
        this.stop();
        Snap.animate(1.2, 1, function (val) {
          _this4.attr({
            transform: "s".concat(val)
          });
        }, 150, mina.easeout);
